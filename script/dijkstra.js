...
export function Dijkstra(arrayData,startNode,endNode,SPEED){
    ...
    while(Queue.length!=0){
        var min = getMinDistance(Queue); //Getting the minimum path (distance) node
        if(min == undefined){
            break;
        }
        // since this element is removed from queue, mark it visited, its distance never updated again
...
function djanimate(data,start,stop,get,speed){
    ...
    //console.log(stop);
    //console.log(stop);
    // data contain visited node ids ("in order of traversal")
    ...
    for (var i = 0; i < data.length; i++) {
        ...
        //console.log(x+"==="+stop);
            setTimeout(function(){
                $("#"+x).addClass("animate");
                //console.log(x);
            },(i+1)*20*speed); // time in milliSeconds
    }
    if(!get){
        setTimeout(function(){
...
function pathAnimate(start,stop,frames,speed){
    ...
    //console.log(trace);
    for (let i = trace.length - 2; i >= 0; i--) { // length-2 because element at length-1 is starting node
        setTimeout(function () {
